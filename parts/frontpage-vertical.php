...
if ($kale_frontpage_vertical_show == 1) {
  $kale_frontpage_vertical_heading = kale_get_option(
    'kale_frontpage_vertical_heading'
  );
  $kale_frontpage_vertical_category = kale_get_option(
    'kale_frontpage_vertical_category'
  );
  global $post;
  $args = array(
    'posts_per_page' => 5,
    'category' => $kale_frontpage_vertical_category
  );
  ...
  if ($vertical_posts) { ?>
    ...
    <div class="frontpage-vertical-posts" id="frontpage-vertical-posts-ej">
        <h2 class="block-title"><span><?php echo esc_html(
          $kale_frontpage_vertical_heading
        ); ?></span></h2>
        <div class="row" data-fluid=".entry-title">
            <?php
            foreach ($vertical_posts as $post) {
              setup_postdata($post); ?>
